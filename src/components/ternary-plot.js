...
export default class TernaryPlot extends React.Component {
  ...
  render() {
    ...
    if (this._plotDom) {
      var offset = {
        top:
          (1.0 - this.state.a) * this._plotDom.offsetHeight -
          this._plotDom.offsetHeight,
        left:
          (P.x / 100.0) * 1.1494 * this._plotDom.offsetHeight -
          this._plotDom.offsetHeight * 1.1494
      };
    ...
    return (
      <div {...other} className="-tp">
        <div className="-tp-labels">
          <p className="-tp-label-a" style={this.computeWeight(this.state.a)}>
            {this.props.labels[0]}
          </p>
          <p className="-tp-label-b" style={this.computeWeight(this.state.b)}>
            {this.props.labels[1]}
          </p>
          <p className="-tp-label-c" style={this.computeWeight(this.state.c)}>
            {this.props.labels[2]}
          </p>
        </div>
        <div
          ref={ref => (this._plotDom = ref)}
          style={plotStyles}
          className="-tp-plot"
        ...
        if (point.x - width / 2 < 0) {
          ...
          I.x =
            (A.x * B.y * C.x -
              ...
              A.x * C.x * point.y +
              A.x * C.y * point.x) /
            (A.x * C.y + B.y * C.x - A.x * point.y - B.y * point.x);
          I.y = (-B.y / A.x) * I.x + B.y;
        }
        // On the right side
        else {
